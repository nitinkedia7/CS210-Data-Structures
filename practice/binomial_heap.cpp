...
node *search_key(node *head, int key) {
	if (head != NULL) {
		if (head->key == key) return head;
		node *ans = NULL;
		
		ans = search_key(head->lchild, key);
		if (ans != NULL) return ans;

		ans = search_key(head->sibling, key);
		if (ans != NULL) return ans;
	}
	return NULL;
}
void binomial_decrease_key(node *head, int oKey, int nKey) {
	node *x = search_key(head, oKey);
	if (x != NULL) {
		node *z = x->parent;
		x->key = nKey;
		while (z != NULL){
...
node *binomial_delete_key(node *head, int key) {
	binomial_decrease_key(head, key, INT_MIN);
	int min_key = binomial_heap_extractMin(&head);
	return head;
}

int main() {
	char cmd; // command
	int arg, nKey;  // argument (if any)
	...
	while (cin >> cmd) {
		if (cmd == '#') cin.ignore(10000, '\n'); // ignore comment line
			...
			cin >> arg >> nKey;
			if (arg > nKey) binomial_decrease_key(head, arg, nKey);
		}
		else if (cmd == 'f') {
			cin >> arg;
			head = binomial_delete_key(head, arg);
		}
	}
}